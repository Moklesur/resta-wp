...
function resta_breadcrumbs( $args = array() ) {
    ...
    $html = '<section id="' . esc_attr( $args['breadcrumbs_id'] ) . '" class="page-breadcrumb text-center ' . esc_attr( $args['breadcrumbs_classes'] ) . '"><div class="container-fluid">';
    ...
    if ( is_singular( 'post' ) ) {
        ...
        if ( class_exists( 'WooCommerce' ) ) {
            if( is_product() ){
                ...
                $html .= '<h2>' . $title['title'] = get_the_title( $shop_page_id ) . '</h1>';
            }else{
        ...
        if ( class_exists( 'WooCommerce' ) ) {
            if( is_shop() ){
                ...
                $html .= '<h2>' . esc_html( $custom_tax_name ) . '</h2>';
                $html .= $home;
                $html .= $separator;
        ...
        $html .= '<h1>'. __( ' Search results for : ', 'resta') . get_search_query() . '</h1>';
        $html .= $home;
        $html .= $separator;
        $html .= '<span class="item-current item-search"><span class="bread-current bread-search">'.__('Search results for:','resta').' ' . get_search_query() . '</span></span>';
    } elseif ( is_404() ) {
        $html .= '<h1>' . __( 'Error 404', 'resta' ) . '</h1>';
        $html .= $home;
        $html .= $separator;
        $html .= '<span>' . __( 'Error 404', 'resta' ) . '</span>';
    } elseif ( is_home() ) {
